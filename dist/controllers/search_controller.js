...
var _resource_model = _interopRequireDefault(require("../models/resource_model"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function search(queryObject, sort, page, numPerPage) {
  return new Promise((resolve, reject) => {
    _resource_model.default.find(queryObject) // Return all results (within limits) if no query
    .sort({
      title: sort === 'a' ? -1 : 1
    }) // Sort by title
...
function soundex(queryString, maxLength = 4) {
  const outputArray = []; // Copy query string, remove all but alphabetical characters, and make uppercase

  const queryStringCopy = queryString.slice().replace(/[^A-Z]/gi, '').toUpperCase(); // Save first letter

  const firstLetter = queryStringCopy.charAt(0); // Iterate over all characters within input string and replace according to the soundex algorithm
