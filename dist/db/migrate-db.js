...
const migrateResources = () => {
  return new Promise((resolve, reject) => {
    ...
    _models.Resources.find({}).exec().then(resources => {
      Promise.all(resources.map(resource => {
        return new Promise((resolve, reject) => {
          _models.Resources.findById(resource._id).then(toUpdateResource => {
            processResource(toUpdateResource).then(updatedResource => {
              modifiedCount += 1;
              updatedResource.save().then(modifiedResource => {
                return resolve(modifiedResource);
              });
            }).catch(() => {
              return resolve(null);
            });
